...
public record QuiverlinkAugment(Priority priority) implements Augment<QuiverlinkAugment>
{
    public static final AugmentType<QuiverlinkAugment> TYPE = new AugmentType<>(
        ...
        QuiverlinkAugment::encode,
        QuiverlinkAugment::decode,
    ...
    private static void encode(FriendlyByteBuf buf, QuiverlinkAugment augment)
    {
        buf.writeEnum(augment.priority);
    }

    private static QuiverlinkAugment decode(FriendlyByteBuf buf)
    {
        Priority priority = buf.readEnum(Priority.class);
        return new QuiverlinkAugment(priority);
    }
