...
public record ItemCollection(HashSet<Identifier> ids)
{
    ...
    public static final Codec<ItemCollection> CODEC = RecordCodecBuilder.create((instance) -> instance.group(
        Identifier.CODEC.sizeLimitedListOf(MAX_ENTRIES).xmap(HashSet::new, ArrayList::new).fieldOf("ids").orElse(new HashSet<>()).forGetter(f -> f.ids)
    ...
    public static final StreamCodec<RegistryFriendlyByteBuf, ItemCollection> STREAM_CODEC = StreamCodec.composite(
        Identifier.STREAM_CODEC.apply(ByteBufCodecs.collection(HashSet::new)), ItemCollection::ids,
        ItemCollection::new
    );
    ...
    public ItemCollection add(Item item)
    {
        if(this.ids.size() >= MAX_ENTRIES)
            return this;
        HashSet<Identifier> filters = new HashSet<>(this.ids);
    ...
    {
        HashSet<Identifier> filters = new HashSet<>(this.ids);
        filters.removeIf(id -> !predicate.test(BuiltInRegistries.ITEM.getValue(id)));
        return new ItemCollection(filters);
    }
    ...
    public ItemCollection limit(int size)
    {
        ...
        if(this.ids.size() <= size)
            return this;
        HashSet<Identifier> set = new HashSet<>(this.ids);
