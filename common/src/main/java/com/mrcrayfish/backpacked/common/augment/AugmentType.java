...
public record AugmentType<T extends Augment<T>>(ResourceLocation id, Codec<T> codec, BiConsumer<FriendlyByteBuf, T> encoder, Function<FriendlyByteBuf, T> decoder, Supplier<T> defaultSupplier, ResourceLocation sprite, Component name, Component description, Supplier<AugmentType<?>> requires) implements Comparable<AugmentType<?>>
{
     ...
     * @param encoder an encoder for synchronization to clients
     * @param decoder a decoder for synchronization to clients
     * @param defaultSupplier a default supplier for the augment value
     */
    public AugmentType(ResourceLocation id, Codec<T> codec, BiConsumer<FriendlyByteBuf, T> encoder, Function<FriendlyByteBuf, T> decoder, Supplier<T> defaultSupplier)
    {
        this(id, codec, encoder, decoder, defaultSupplier, () -> null);
    }
     ...
     */
    public AugmentType(ResourceLocation id, Codec<T> codec, BiConsumer<FriendlyByteBuf, T> encoder, Function<FriendlyByteBuf, T> decoder, Supplier<T> defaultSupplier, Supplier<AugmentType<?>> requires)
    {
        this(id, codec, encoder, decoder, defaultSupplier,
            new ResourceLocation(id.getNamespace(), "augment/%s".formatted(id.getPath())),
