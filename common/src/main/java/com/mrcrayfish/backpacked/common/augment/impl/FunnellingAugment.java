...
public record FunnellingAugment(ItemCollection filters, Mode mode) implements Augment<FunnellingAugment>, FilterableItems<FunnellingAugment>
{
    public static final AugmentType<FunnellingAugment> TYPE = new AugmentType<>(
        ...
        FunnellingAugment::encode,
        FunnellingAugment::decode,
    ...
    private static void encode(FriendlyByteBuf buf, FunnellingAugment augment)
    {
        augment.filters.encode(buf);
        buf.writeEnum(augment.mode);
    }

    private static FunnellingAugment decode(FriendlyByteBuf buf)
    {
        ItemCollection filters = ItemCollection.decode(buf);
        Mode mode = buf.readEnum(Mode.class);
        return new FunnellingAugment(filters, mode);
    }
