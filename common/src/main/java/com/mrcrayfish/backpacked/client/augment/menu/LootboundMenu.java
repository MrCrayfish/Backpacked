...
public class LootboundMenu extends AugmentSettingsMenu
{
    ...
    public LootboundMenu(PopupMenuHandler handler, AugmentHolder<LootboundAugment> holder)
    {
        super(handler, menu -> {
            ...
            rootHelper.addChild(createOption(BLOCKS_LABEL, BLOCKS_TOOLTIP, BackpackButtons.onOff(() -> holder.get().blocks(), value -> holder.update(holder.get().setBlocks(value))).setSize(60, 18).build(), divider.getWidth()));
            rootHelper.addChild(createOption(MOBS_LABEL, MOBS_TOOLTIP, BackpackButtons.onOff(() -> holder.get().mobs(), value -> holder.update(holder.get().setMobs(value))).setSize(60, 18).build(), divider.getWidth()));
            return root;
        });
    }
}
