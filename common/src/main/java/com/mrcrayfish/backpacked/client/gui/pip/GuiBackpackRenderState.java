...
{
    public GuiBackpackRenderState(@Nullable ItemTransform transform, @Nullable BackpackRenderer renderer, @Nullable LivingEntityDataState entityData, @Nullable LevelDataState levelData, Identifier baseModel, Identifier strapsModel, int entityId, int tickCount, float partialTick, Matrix3x2f pose, int x0, int x1, int y0, int y1, float scale, @Nullable ScreenRectangle scissorArea)
    {
        this(transform, renderer, entityData, levelData, baseModel, strapsModel, entityId, tickCount, partialTick, pose, x0, x1, y0, y1, scale, scissorArea, PictureInPictureRenderState.getBounds(x0, y0, x1, y1, scissorArea));
    }
}