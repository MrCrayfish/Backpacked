...
public abstract class UnlockableContainerScreen<T extends AbstractContainerMenu> extends CustomContainerScreen<T> // TODO DONE
{
        ...
        for(Slot slot : this.getMenu().slots)
        {
            if(slot instanceof UnlockableSlot lockedSlot)
            {
                ...
                if(!lockedSlot.isUnlocked())
                {
                    ...
                    {
                        TextureDefinitions.LOCK_OUTLINED.draw(graphics, this.leftPos + slot.x + 1, this.topPos + slot.y + 1, 14, 14);
                    }
                    else
                    {
                        TextureDefinitions.LOCK.draw(graphics, this.leftPos + slot.x + 2, this.topPos + slot.y + 2, 12, 12);
    ...
    protected void spawnSlotUnlockedParticles(int slotX, int slotY)
    {
        Particle2D top = new Particle2D(slotX + 2, slotY + 2, 12, 6)
            .setLife(50)
            .setSprite(TextureDefinitions.WIDGETS_LOCATION, 244, 146, 12, 6)
        ...
        Particle2D bottom = new Particle2D(slotX + 2, slotY + 8, 12, 6)
            .setLife(50)
            .setSprite(TextureDefinitions.WIDGETS_LOCATION, 244, 152, 12, 6)
        ...
        for(int i = 0; i < 10; i++)
        {
            Particle2D damageParticle = new Particle2D(slotX + 7, slotY + 7, 2, 2)
                .setLife(20)
                .setSprite(TextureDefinitions.WIDGETS_LOCATION, 248, 165, 2, 2)
        ...
        if(Config.CLIENT.glitterBomb.get())
        {
            for(int i = 0; i < 200; i++)
            {
                Particle2D expOrbParticle = new Particle2D(slotX + 6, slotY + 6, 4, 4)
                    .setLife(50)
                    .setSprite(TextureDefinitions.WIDGETS_LOCATION, 117, 1, 9, 9)
