...
public class BackpackShelfScreen extends UnlockableContainerScreen<BackpackShelfMenu>
{
    ...
    public void renderBg(GuiGraphics graphics, float partialTick, int mouseX, int mouseY)
    {
        ...
        Slot shelfSlot = this.getMenu().slots.get(0);
        ...
        TextureDefinitions.BACKPACK_BACKGROUND.draw(graphics, this.leftPos, this.topPos + 48, this.imageWidth, 43);

        // Draws a slots for the backpacks
        Slot backpacksFirstSlot = this.getMenu().slots.get(1);
        TextureDefinitions.BACKPACK_SLOT.draw(graphics, this.leftPos + backpacksFirstSlot.x - 1, this.topPos + backpacksFirstSlot.y - 1, this.menu.getManagementContainer().getContainerSize() * 18, 18);
