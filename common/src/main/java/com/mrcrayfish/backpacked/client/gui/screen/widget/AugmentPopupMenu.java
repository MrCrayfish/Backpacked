...
public class AugmentPopupMenu extends PopupMenu
{
    ...
    public AugmentPopupMenu(PopupMenuHandler handler, Supplier<Augments> selectedAugments, Consumer<Augment<?>> updater, boolean showHidden)
    {
        ...
        TitleWidget title = this.helper.addChild(new TitleWidget(Component.translatable("backpacked.gui.pick_an_augment"), Minecraft.getInstance().font));
        Divider divider = this.helper.addChild(Divider.horizontal(Math.max(grid.getWidth(), title.getWidth())).colour(0xFFE0CDB7));
        title.setWidth(divider.getWidth());
        this.helper.addChild(grid, LayoutSettings.defaults().alignHorizontallyLeft());
    }
