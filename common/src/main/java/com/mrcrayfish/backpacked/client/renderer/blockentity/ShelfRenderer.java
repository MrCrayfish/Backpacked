...
public class ShelfRenderer implements BlockEntityRenderer<ShelfBlockEntity> // TODO DONE
{
    ...
    public void render(ShelfBlockEntity entity, float partialTick, PoseStack pose, MultiBufferSource buffer, int light, int overlay)
    {
        ...
        CosmeticProperties properties = CosmeticProperties.view(stack, CosmeticProperties.DEFAULT);
        String cosmeticId = properties.cosmetic().orElse(BackpackManager.getDefaultOrFallbackCosmetic());
    ...
    private void renderBackpackName(ShelfBlockEntity shelf, Direction facing, PoseStack poseStack, MultiBufferSource source, int light)
    {
        ...
        if(mc.hitResult instanceof BlockHitResult result && result.getBlockPos().equals(shelf.getBlockPos()))
        {
            ...
            if(!backpack.isEmpty() && backpack.hasCustomHoverName())
            {
                Component label = backpack.getHoverName();
