...
public class RecallMenu extends AugmentSettingsMenu
{
    ...
    public RecallMenu(PopupMenuHandler handler, AugmentHolder<RecallAugment> holder)
    {
        super(handler, menu -> {
            ...
            ShelfStatus status = rootHelper.addChild(new ShelfStatus(MIN_CONTENT_WIDTH - 2, 16, holder), LayoutSettings.defaults().alignHorizontallyCenter());
            status.check();
            rootHelper.addChild(BackpackButtons.builder()
    ...
    public static class ShelfStatus extends AbstractWidget
    {
        ...
        private static final FrameworkTexture SHELF_STATUS_DEFAULT_TEXTURE = FrameworkTexture.nineSlice(TextureDefinitions.WIDGETS_LOCATION, 102, 0, 14, 14, Border.of(4));
        private static final FrameworkTexture SHELF_STATUS_INVALID_TEXTURE = FrameworkTexture.nineSlice(TextureDefinitions.WIDGETS_LOCATION, 74, 0, 14, 14, Border.of(4));
        private static final FrameworkTexture SHELF_STATUS_VALID_TEXTURE = FrameworkTexture.nineSlice(TextureDefinitions.WIDGETS_LOCATION, 88, 0, 14, 14, Border.of(4));
        ...
        {
            this.getStatusSprite().draw(graphics, this.getX(), this.getY(), this.getWidth(), this.getHeight());
        ...
        private FrameworkTexture getStatusSprite()
        {
            return switch(this.status) {
                case NOT_SET -> SHELF_STATUS_DEFAULT_TEXTURE;
                case INVALID -> SHELF_STATUS_INVALID_TEXTURE;
                case VALID -> SHELF_STATUS_VALID_TEXTURE;
            };
        }
