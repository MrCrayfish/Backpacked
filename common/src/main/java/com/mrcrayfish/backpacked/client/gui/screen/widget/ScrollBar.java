...
{
    private static final WidgetTextures SCROLL_BAR_SPRITES = new WidgetTextures(
        TextureDefinitions.SCROLLER,
        TextureDefinitions.SCROLLER_DISABLED
    );
    ...
    protected void renderWidget(GuiGraphics graphics, int mouseX, int mouseY, float partialTick)
    {
        ...
        SCROLL_BAR_SPRITES.get(this.active, false).draw(graphics, this.getX(), scrollBarY, SCROLL_BAR_WIDTH, SCROLL_BAR_HEIGHT);
    }
    ...
    {
        int maxScroll = this.getMaxScroll();
        int newScroll = Mth.ceil(maxScroll * Mth.clamp(scrollAmount, 0, 1));
