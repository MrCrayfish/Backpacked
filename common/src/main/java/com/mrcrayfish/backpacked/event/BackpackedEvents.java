...
public class BackpackedEvents
{
    ...
    public static final FrameworkEvent<InteractedWithBlock.CaptureTag> INTERACTED_WITH_BLOCK_CAPTURE_TAG = new FrameworkEvent<>(listeners -> (player) -> {
        for(var listener : listeners) {
            if(listener.handle(player)) {
                return true;
            }
    ...
    public static final FrameworkEvent<InteractedWithBlock> INTERACTED_WITH_BLOCK = new FrameworkEvent<>(listeners -> (snapshot, stack, player) -> {
        listeners.forEach(listener -> listener.handle(snapshot, stack, player));
    });
